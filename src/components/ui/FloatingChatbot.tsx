...
const starterMessages: ChatMessage[] = [
  {
    id: 1,
    author: "assistant",
    text: "Hi, I’m Hive. I can help you explore BrandHive Studio’s services, explain our process, or guide you to the Project Planner.",
  },
];
          ...
          >
            <div className="relative flex items-center justify-between border-b border-white/8 px-5 py-4">
              ...
              <div className="relative flex items-center gap-3">
                <div className="grid size-10 place-items-center rounded-2xl border border-[#16C7FF]/30 bg-[#16C7FF]/10 text-[#16C7FF] shadow-[0_0_20px_rgba(22,199,255,0.15)]">
                  <Bot className="size-5" aria-hidden="true" />
                </div>
                <div>
                  <p className="text-sm font-semibold tracking-tight text-white">Hive Assistant</p>
                  <p className="flex items-center gap-1.5 text-xs text-white/55"><span className="size-1.5 rounded-full bg-[#16C7FF] shadow-[0_0_8px_#16C7FF]" />Premium concierge</p>
                </div>
              </div>
              <button type="button" onClick={() => setIsOpen(false)} className="relative grid size-9 place-items-center rounded-full text-white/60 transition hover:bg-white/8 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16C7FF]" aria-label="Close chat">
            ...
            >
              {messages.map((message) => (
                <motion.div key={message.id} initial={shouldReduceMotion ? false : { opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={message.author === "visitor" ? "ml-auto max-w-[85%]" : "max-w-[88%]"}>
                  <p className={message.author === "visitor" ? "rounded-2xl rounded-br-md bg-[#16C7FF] px-3.5 py-2.5 text-sm leading-5 text-[#050608] shadow-[0_6px_18px_rgba(22,199,255,0.2)]" : "rounded-2xl rounded-bl-md border border-white/8 bg-white/5 px-3.5 py-2.5 text-sm leading-5 text-white/80"}>
                    {message.text}
                  </p>
                </motion.div>
              ))}
            ...
            <form onSubmit={handleSubmit} className="border-t border-white/8 p-3">
              ...
              <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/20 p-1.5 transition focus-within:border-[#16C7FF]/45 focus-within:ring-4 focus-within:ring-[#16C7FF]/10">
                <input ref={inputRef} id="chat-message" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={handleKeyDown} placeholder="Type your message..." className="min-w-0 flex-1 bg-transparent px-2.5 py-2 text-sm text-white outline-none placeholder:text-white/35" autoComplete="off" />
              ...
              <p className="pt-2 text-center text-[10px] text-white/35">Powered by BrandHive Studio</p>
            </form>
          </motion.section>
        )}
      </AnimatePresence>

      <motion.button type="button" onClick={() => setIsOpen((open) => !open)} whileHover={shouldReduceMotion ? undefined : { scale: 1.03 }} whileTap={shouldReduceMotion ? undefined : { scale: 0.96 }} className="group relative flex items-center gap-3 overflow-hidden rounded-full border border-[#16C7FF]/30 bg-[#061018]/95 px-4 py-3 shadow-[0_18px_70px_rgba(0,0,0,0.45),0_0_20px_rgba(22,199,255,0.18)] transition duration-300 hover:border-[#16C7FF]/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#16C7FF] focus-visible:ring-offset-4 focus-visible:ring-offset-[#050608]" aria-label={isOpen ? "Minimize Hive Assistant" : "Talk to Hive Assistant"} aria-expanded={isOpen} aria-controls="brandhive-chat">
        <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#0b1521]/80 border border-[#16C7FF]/15 shadow-[0_0_18px_rgba(22,199,255,0.12)]">
          <svg viewBox="0 0 24 24" className="h-5 w-5 text-[#16C7FF]" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round">
            <path d="M12 2.5l4.8 2.8v5.6l-4.8 2.8-4.8-2.8V5.3L12 2.5z" />
            <path d="M12 12.4l4.8 2.8v5.6l-4.8 2.8-4.8-2.8v-5.6l4.8-2.8z" />
          </svg>
        </span>
        <span className="flex flex-col items-start text-left">
          <span className="text-sm font-semibold text-white">Talk to Hive</span>
          <span className="text-[11px] uppercase tracking-[0.22em] text-[#7de0ff]/70">Assistant</span>
        </span>
      </motion.button>
    </div>
  </>
